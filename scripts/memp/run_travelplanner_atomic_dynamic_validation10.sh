...
RESULTS="$ROOT/.runtime/memp/travelplanner/atomic_dynamic_validation10/results"
RETRIEVAL="$ROOT/data/memp_ncf/travelplanner_atomic_dynamic_validation10/retrieval.json"
...
"$PYTHON" "$ROOT/scripts/memp/evaluate_travelplanner_validation_slice.py" \
  --limit 10 \
  --results-dir "$RESULTS" \
  --output "$ROOT/.runtime/memp/travelplanner/atomic_dynamic_validation10/evaluation.json"
