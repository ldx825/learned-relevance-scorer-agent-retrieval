#!/usr/bin/env bash
# 24-worker run of the 180-task Proceduralization paired run (41-doc quality
# pool): 2 arms x 12 slices of 15 tasks.  The runner skips saved tasks, so
# parallelism never re-pays finished work.
set -euo pipefail
...
BASE="$ROOT/private/memp/travelplanner/v25_proceduralization_q41_validation180_paired_run"
SLICES="$ROOT/private/memp/travelplanner/v25_proceduralization_q41_180_retrieval_slices24"

mkdir -p "$BASE/cosine/results" "$BASE/neumf/results"
PIDS=()
for arm in cosine neumf; do
  for part in $(seq 0 11); do
